...
import {
  ...
  CheckCircle,
  AlertTriangle,
  Mail, // Mail icon is imported but not used, can be removed if not needed.
} from "lucide-react"; // Import ikon
...
const SignUp = () => {
  ...
  const handleRegister = async (e) => {
    ...
    // This is where the error occurred because validateEmail was not defined
    if (!validateEmail(email)) {
      setError(
        "Format email tidak valid. Harap masukkan email dengan format yang benar."
      );
      setLoading(false);
      return;
    }
    if (!validatePhoneNumber(phoneNumber)) {
      setError(
        "Format nomor telepon tidak valid. Pastikan nomor telepon diawali dengan 08, 62, atau +62 dan terdiri dari 8 hingga 13 angka."
      );
      setLoading(false);
      return;
    }
