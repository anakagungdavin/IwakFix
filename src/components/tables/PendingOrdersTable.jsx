...
const PendingOrdersTable = () => {
  ...
  const handleApprove = async (orderId) => {
    try {
      ...
      // Close modal and refresh the list after approval
  ...
  const handleReject = async (orderId) => {
    try {
      ...
      // Close modal and refresh the list after rejection
      setImageModalOpen(false);
      fetchPendingOrders();
    } catch (err) {
  ...
  const viewPaymentProof = (order) => {
    // For now, use the first product image as a placeholder for payment proof
    const paymentProofImage = order.items[0]?.product?.images?.[0] || "";
    setSelectedImage(paymentProofImage);
  ...
  const formatDate = (dateString) => {
    ...
    if (windowWidth < 640) {
      // Shorter format for small screens: DD/MM/YY
  ...
  // Function to truncate product names based on screen size
  const truncateText = (text, maxLength) => {
    const length = windowWidth < 640 ? Math.min(maxLength, 20) : maxLength;
    return text.length > length ? `${text.substring(0, length)}...` : text;
  };

  // Function to format order ID based on screen size
  const formatOrderId = (id) => {
    if (windowWidth < 640) {
      // Show only first and last 4 characters for small screens
    ...
    <div className="w-full">
      ...
      {pendingOrders.length === 0 ? (
        ...
        <div className="relative bg-white rounded-lg shadow-sm">
          {/* Show indicator on small screens that table is scrollable */}
